...
router.get('/', function(req, res, next) {
    ...
    request.post(glbpath+"financingProject.action",function callback(err,httpResponse,body){
        if(err){
        ...
        try{
            ...
            for(var i=0;i<projectList.length;i++){
                ...
                var remainDay=GlbParam.remainDay(projectList[i].financeEndTime)
                projectList[i]["remainDay"]=remainDay;
            }

            res.render('projectfinance', {
...
router.post('/conPage', function(req, res) {
    var data=req.body;
    request.post(glbpath+"financingProject.action?pageNo="+data.pageNo,function callback(err,httpResponse,body){
        if(err){
            return console.error("error："+err);
        }
        try{
