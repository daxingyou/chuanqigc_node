/*!
 * 注意：该文件是公用控件，由编译工具自动生成，不允许任何手工调整！！！
 * 如需调整该文件代码，请联系前端架构中心。
 */
...
(function($, window, undefined) {
    ...
    var _box, _thisScript, _skin, _path,
        ...
        __cssPath = 'http://core.pc.lietou-static.com/css/port/lpt/plugins/dialog/default.css',
        __imgPath = 'http://core.pc.lietou-static.com/images/port/lpt/plugins/dialog/default/';
    ...
    var artDialog = function(config, ok, cancel) {
        ...
        $.each({
            ...
            config[i] = config[i] !== undefined && config[i] !== null ? config[i] : config[o];
        });
        // 返回跟随模式或重复定义的ID
        ...
        config.cancel && config.button.push({
            ...
            className: "aui-btn aui-btn-light",
    ...
    artDialog.fn = artDialog.prototype = {
        ...
        _init: function(config) {
            var that = this,
                ...
                iconBg = icon && (_isIE6 ? {
                    ...
                    backgroundImage: 'url(' + __imgPath + 'icons/' + icon + '.png)'
                });
        ...
        content: function(msg) {
            ...
            // 对dialog-buttons做处理，设置padding为0，绑定取消按钮事件
            if ($content.find('.dialog-buttons').length > 0) {
                $content.css('padding', 0);
                $content.find('.dialog-buttons .aui-btn-cancel').bind('click', function() {
        ...
        position: function(left, top) {
            ...
            if (navigator.userAgent.indexOf('MSIE 8.0') > -1) {
                $(that.DOM.main).attr('style', '').attr('style', 'width:auto;height:auto;');
            }
            return that;
        },
        ...
        button: function() {
            var that = this,
                ...
                buttons = DOM.buttons,
                elem = buttons[0],
                strongButton = 'aui-btn aui-btn-primary',
        ...
        time: function(second, show) {
            ...
            if (second) {
                if (show) {
                    var template = "* 秒之后自动关闭",
                        start = second,
                        timerWrap = $("<div class=\"timer\"></div>"),
                        intervalit = function() {
                            timerWrap.html(template.replace(/\*/g, start--));
                            start < 0 && clearInterval(intervalTimer);
                        };
                    if (typeof show == "string") template = sohw;
                    timerWrap.appendTo((that.content()));
                    intervalit();
                    intervalTimer = setInterval(intervalit, 1000)
                }
        ...
        lock: function() {
            ...
            var that = this,
                ...
                //lockMaskWrap = that._lockMaskWrap|| $(prependChild(_$body[0],document.createElement('div')),
                lockMaskWrap = that._lockMaskWrap || $('<div />').prependTo('body'),
            ...
            if (that.config.dblclose) lockMask.bind('dblclick', function() {
                that._click(that.config.cancelVal);
            });
            ...
            DOM.wrap
                .bind('click', function(event) {
                    ...
                    if (target === DOM.close[0]) {
                        ...
                        //修正 chrome浏览器获取target不为button的bug，chenbl
                        if (!$(target).is("button") && $(target).parents(".aui_buttons").length > 0) target = target.parentNode.parentNode;
...
(function($) {
    ...
    _$document.bind('mousedown', function(event) {
        ...
        if (config.drag !== false && target === DOM.title[0] || config.drag !== false && target.getAttribute('data-dragable') === 'dragable' || config.resize !== false && target === DOM.se[0]) {
...
(function($, window, artDialog, undefined) {
     ...
     * @param {Number}  显示时间 (默认1.5秒)
     */
    artDialog.frize = function(content, time, callback) {
        ...
        return _proxyDialog(options).content('<div style="padding: 0 1em;">' + content + '</div>');
    };

    /** 关闭全部 */
    artDialog.closeAll = function() {
        for (var i in artDialog.list) {
            artDialog.list[i].close();
        };
        return artDialog;
    };
    ...
    /** 获取最顶层Dialog */
    artDialog.topDialog = function() {
        var ii = null;
        for (var i in artDialog.list) {
            if (/^artDialog\d+$/.test(i)) ii = i;
        };
        return artDialog.list[ii];
    };

    /** 获取父层Dialog */
    artDialog.parentDialog = function(deep) {
        var ii = null,
            len = 0,
            deep = deep || 1,
            j = 0;
